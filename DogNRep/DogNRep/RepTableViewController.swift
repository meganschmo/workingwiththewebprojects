...
class RepTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        func fetchMatchingRep() {
            ...
            if !searchTerm.isEmpty {
                ...
                Task {
                    do {
                        let repItems = try await representativeController.fetchRepresentative(matching: query)
                        DispatchQueue.main.async {
                                            self.representative = repItems
                                            self.tableView.reloadData()
                                        }
                    } catch {
    ...
    func configure(cell: RepTableViewCell, forItemAt indexPath: IndexPath) {
        ...
        cell.name = rep.repName
        cell.party = rep.repParty
        cell.link = rep.repLink
    }
